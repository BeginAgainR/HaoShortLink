...
- v1.2 工程化运行进行中，已新增并验证本地 Docker Compose 编排，可启动 MySQL、Redis 和 `shortlink_server` 服务容器。
...
当前短链接服务支持内存存储、MySQL 持久化和可选 Redis 查询缓存；已提供并验证本地 Docker Compose 编排，可启动 MySQL、Redis 和 `shortlink_server` 服务容器；Nginx、消息队列和监控能力尚未实现。
