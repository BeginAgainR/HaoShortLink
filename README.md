...
- v1.3 已新增第一批 CTest、API 冒烟测试、MySQL / Redis 集成测试脚本和 CI 第一版 workflow；GitHub Actions 云端结果待推送后确认。
...
当前短链接服务支持内存存储、MySQL 持久化和可选 Redis 查询缓存；已提供并验证本地 Docker Compose 编排，可启动 MySQL、Redis、`shortlink_server` 和 Nginx；已新增第一批自动化测试和 CI 第一版 workflow。消息队列和监控能力尚未实现。
