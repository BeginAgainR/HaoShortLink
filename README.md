...
- v1.6 已完成 Redis Lua 全局创建限流、fail-open、liveness / readiness 和保护性测试的本地收口；云端 CI 待开发分支推送后确认。
...
当前短链接服务支持内存存储、MySQL 持久化、可选 Redis 查询缓存和可选 Redis 全局创建限流；已提供并验证本地 Docker Compose 编排，可启动 MySQL、Redis、`shortlink_server`、Nginx、Prometheus 和 Grafana；v1.3 已完成第一版自动化测试和 CI 收口，v1.4 已完成性能与稳定性基线，v1.5 已完成日志、指标、采集和最小 dashboard，v1.6 已完成本地回归与干净克隆验证。消息队列尚未实现。
