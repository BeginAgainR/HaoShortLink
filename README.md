...
- v1.5 进行中：已完成 request ID、通用结构化请求日志、基础 HTTP / 短链指标、`/metrics`，以及本地 Prometheus、Grafana 和最小 dashboard。
- MySQL / Redis 依赖集成已进入 CI；下一批 v1.5.4 聚焦自动化验证、性能回归和文档收口。
...
当前短链接服务支持内存存储、MySQL 持久化和可选 Redis 查询缓存；已提供并验证本地 Docker Compose 编排，可启动 MySQL、Redis、`shortlink_server`、Nginx、Prometheus 和 Grafana；v1.3 已完成第一版自动化测试和 CI 收口，v1.4 已完成性能与稳定性基线，v1.5 已完成日志、指标、采集和最小 dashboard。消息队列尚未实现。
