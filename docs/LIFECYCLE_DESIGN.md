...
v2.0 说明：本文保留 v1.7 的历史设计。v2.0 已移除 `/internal/short-links...` 路由，并以需要登录、
按 `owner_id` 授权的 `/api/short-links...` 管理接口取代；生命周期和缓存语义保持不变。
...
用户身份、链接归属和正式管理 API 已在 v2.0 完成；v2.0 不实现 RBAC 或平台管理员。
