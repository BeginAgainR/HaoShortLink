...
| v1.2 | 进行中 | 工程化运行 | Docker Compose、Nginx、配置样例、部署文档 |
| v1.3 | 草案 | 测试与 CI | 框架测试、API 测试、集成测试、CI |
...
1. Docker Compose：
   - 状态：尚未实现。
   - 编排 `shortlink_server`、MySQL、Redis。
   - 提供本地一键启动和停止方式。
2. 配置整理：
   - 状态：进行中。
   - 已补充 Linux VM 手工运行方式、配置要点、接口验证和常见故障排查。
   - 后续继续明确 Docker Compose 场景下 app、MySQL、Redis 的配置样例。
   - 避免把本机临时配置写死到代码中。
3. Nginx：
   - 状态：尚未实现。
   - 提供统一 HTTP 入口和反向代理配置。
   - 验证短链创建和跳转路径经 Nginx 转发仍然正常。
4. 文档与验证：
   - 状态：尚未实现。
   - 更新部署文档和运行手册。
   - 做干净克隆或干净环境验证。
...
3. 集成测试：
   - 状态：尚未实现。
   - 覆盖 MySQL 持久化、Redis 命中/未命中、Redis 不可用回退 MySQL。
4. CI：
   - 状态：尚未实现。
   - 在远端环境中自动执行构建和基础测试。
