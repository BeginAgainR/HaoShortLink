...
4. v1.4.3 MySQL 创建路径并发失败诊断：
   - 状态：尚未实现。
   - 针对 BUG-004 补充可复现诊断入口，保留状态码分布、响应体样本和服务日志。
   - 按并发 1、2、4、8、16 逐步验证 `POST /api/short-links` 的失败边界。
   - 先确认失败类型和根因，再决定是否修改 MySQL repository、数据库连接池或 HTTP 请求解析。
5. v1.4.4 异常场景补强：
   - 状态：尚未实现。
   - 覆盖 Redis 不可用、MySQL 不可用、非法请求高并发和短码不存在高并发。
   - 确认错误响应稳定，服务不崩溃，不出现明显挂死。
6. v1.4.5 连接与资源评估：
   - 状态：尚未实现。
   - 优先排查 Redis hit 和 missing-code 路径异常慢的问题。
   - 根据压测结果评估 Redis 连接复用或连接池。
   - 根据压测结果评估 MySQL 连接池参数和 worker 线程数量关系。
   - 评估连接池等待是否需要超时机制。
7. v1.4.6 压测工具基线补强：
   - 状态：尚未实现。
