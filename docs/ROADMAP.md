...
| v1.4 | 进行中 | 性能与稳定性 | 压测、连接资源评估、错误场景覆盖、Nginx 入口底线验证 |
| v1.5 | 草案 | 可观测性 | 结构化日志、指标统计、Prometheus、Grafana |
| v1.6 | 草案 | 流量保护 | Redis 限流、限流配置、降级策略、保护性测试 |
| v1.7 | 草案 | 消息队列与异步事件 | Kafka、Kafka UI、事件生产与消费、访问事件流 |
| v1.8 | 草案 | 访问统计与数据查询 | 统计聚合、统计表、访问报表 API、异步一致性验证 |
| v1.9 | 草案 | 工程化增强 | 依赖集成 CI、配置分层、发布检查、部署文档增强 |
| v2.0 | 草案 | 功能相对齐全的短链接服务 | 用户系统、链接管理、过期策略、访问统计、后台管理 |
...
当前状态：进行中，已完成 v1.4.0 文档定界、v1.4.1 压测脚本入口、v1.4.2 第一轮 curl fallback 多模式基线、v1.4.3 MySQL 创建路径修复、v1.4.4 异常场景补强、v1.4.5 Redis 环境问题定性与配置策略验证和 v1.4.6 `hey` / Nginx 入口底线验证；连接池和限流实现不作为 v1.4 默认目标。
...
8. v1.4.7 回归验证与版本收口：
   ...
   - 记录 Redis 地址策略验证结论，以及是否需要 Redis 连接池、MySQL 连接池参数矩阵或连接等待超时。
   - 明确限流正式实现进入 v1.6，不在 v1.4 内继续扩展。
   - 更新 `docs/BENCHMARK.md`、`docs/BUGS.md` 和本路线图，将 v1.4 标记为已完成。
...
v1.6 目标是为服务补充基础流量保护能力。该阶段优先引入 Redis 作为限流状态存储，
避免只写进程内限流后无法覆盖多实例场景。
...
v1.7 目标是引入消息队列，将短码跳转产生的访问事件从同步请求路径中拆出，为后续访问统计做准备。
...
v1.8 目标是将 v1.7 的访问事件落成可查询的业务能力。
...
v1.9 目标是把已经引入的 MySQL、Redis、Nginx、Prometheus、Grafana 和 Kafka 等依赖整理成更稳定的工程化运行方式。

1. 依赖集成 CI：
   - 状态：尚未实现。
   - 评估在 CI 中启动 MySQL、Redis、Kafka 等依赖并执行核心集成测试。
2. 配置分层：
   - 状态：尚未实现。
   - 区分本地、测试和容器内配置样例。
3. 发布检查：
   - 状态：尚未实现。
   - 形成构建、测试、迁移、回滚和文档检查清单。
...
v2.0 是第一个功能相对齐全的短链接服务里程碑。该版本不追求生产级高可用，
但应具备完整业务闭环、基础管理能力和正经的自动化验证。
