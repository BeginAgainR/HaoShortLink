...
| v1.9 | 本地实现完成，待云端 CI | 访问统计与工程化增强 | 幂等统计、查询 API、重试与 DLQ、Lag、重放与发布检查 |
...
- v2.1 不是 v2.0 验收前置条件；只有出现可靠发布生命周期事件或多消费者 schema 治理需求时才启动。
...
当前状态：本地实现、完整 Compose 故障回归和独立干净目录验证已完成；待分支提交后的 GitHub Actions 确认。详细边界见 `docs/ACCESS_STATISTICS_DESIGN.md`。
...
7. `v1.9.6` 工程化与发布收口：
   - 状态：配置、Compose、CI 入口、运行手册和独立干净目录全量回归已完成；待云端 CI 确认。
...
当前状态：条件候选，不是 v2.0 前置条件。

v2.1 只在出现明确需求后启动：

1. 生命周期事件可靠发布：
   - 对链接创建、禁用、恢复、过期时间或归属变更等本来就写 MySQL 的操作，评估 Transactional Outbox。
   - 访问事件来自读路径，不为每次跳转增加 Outbox 同步写入。
2. schema 治理：
   - 只有出现多个跨语言消费者或频繁 schema 演进后，才评估 Schema Registry、Avro 或 Protobuf。
3. 验证边界：
   - 覆盖业务表与 outbox 原子提交、发布重试、重复发布和消费者兼容性。
