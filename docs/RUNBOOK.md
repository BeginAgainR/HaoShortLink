...
v1.7 内部生命周期接口同样不经过 Nginx；通过 `8080` 访问 `/internal/` 应返回 `404`。
本机运维调试可以通过只绑定 `127.0.0.1` 的 `18080` 直连端口访问：

```bash
curl -i http://127.0.0.1:18080/internal/short-links/000001
```
...
mysql -u hao_shortlink -p hao_shortlink < /Users/hao/Code/haoHTTP/apps/shortlink_server/sql/003_add_short_link_lifecycle.sql
```

已有数据库只执行尚未应用的 `003_add_short_link_lifecycle.sql`。该迁移默认将已有记录设为
`active` 且保持 `expires_at = NULL`。迁移文件是一次性版本脚本，不应在同一数据库重复执行。
...
启用统计前先应用 `apps/shortlink_server/sql/004_create_access_statistics.sql`。空数据卷会按文件名顺序自动执行
`001` 至 `004`；已有数据卷需要显式执行 `004`。服务端仅在以下组合下注册统计 API：
