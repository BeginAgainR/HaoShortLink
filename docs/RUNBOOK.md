...
状态：v2.0 本地运行、迁移和验证入口已建立，持续维护
当前实现：支持 Linux VM 构建、Compose 产品环境、独立 schema migration、同源管理页面和完整回归；
不在 Mac 宿主机上构建 C++ 项目。
...
auth.session_ttl_seconds=604800
...
- `memory` 是默认存储方式。
- `mysql` 需要先通过迁移入口建立 001-005 schema 并配置 MySQL 连接信息。
- `auth.registration_enabled` 控制开放注册，默认开启。
- `auth.session_ttl_seconds` 允许 300 到 2592000 秒，非法值回退为 604800 秒。
- `auth.cookie_secure` 在 HTTPS 部署时必须为 `true`；本地 HTTP 示例为 `false`。
...
- MySQL 空数据卷会按文件名顺序执行 001-005；已有卷由 `schema_migrate` 检测并升级。
- `shortlink_server` 只在 migration 成功退出后启动；版本 5 已存在时仍验证关键表、owner、索引和外键。
...
```bash
curl -i -c /tmp/hao-shortlink-cookies.txt \
  -H 'Content-Type: application/json' \
  -d '{"username":"demo_user","password":"demo-password"}' \
  http://127.0.0.1:8080/api/auth/register

curl -i -b /tmp/hao-shortlink-cookies.txt \
  -H 'Content-Type: application/json' \
  -d '{"url":"https://example.com/compose-container","custom_code":"compose_demo"}' \
...
```bash
curl -i http://127.0.0.1:8080/s/compose_demo
```
...
v2.0 已移除旧内部生命周期和统计路由。Nginx 与应用直连的 `/internal/` 都应返回 `404`；正式管理使用
需要 Cookie 和 owner 授权的 `/api/short-links...`：

```bash
curl -i http://127.0.0.1:8080/internal/short-links/compose_demo
curl -i http://127.0.0.1:18080/internal/short-links/compose_demo
```
...
HAOHTTP_MYSQL_USER=hao_shortlink \
HAOHTTP_MYSQL_PASSWORD=<password> \
HAOHTTP_MYSQL_DATABASE=hao_shortlink \
bash tests/scripts/migrate_shortlink_schema.sh up
```

不要在已有库上手工重放所有 SQL。迁移入口识别空库或受支持的 v1.9 schema，应用 005、回填历史 owner，
并验证版本记录与关键约束。Compose 可直接运行 `docker compose run --rm schema_migrate`。
...
v1.7 起查询缓存 TTL 必须在 1 到 86400 秒之间；非法值回退为默认的 3600 秒。
...
curl -i -c /tmp/hao-shortlink-cookies.txt \
  -H 'Content-Type: application/json' \
  -d '{"username":"manual_user","password":"manual-password"}' \
  http://127.0.0.1:8080/api/auth/register
```
...
curl -i -b /tmp/hao-shortlink-cookies.txt \
  -H 'Content-Type: application/json' \
  -d '{"url":"https://example.com/very/long/path","custom_code":"manual_demo"}' \
  http://127.0.0.1:8080/api/short-links
```
...
```bash
curl -i http://127.0.0.1:8080/s/manual_demo
```
...
- `schema_migrate` 是否成功执行到版本 5，`users`、`user_sessions` 和 `short_links.owner_id` 是否存在。
...
- 客户端是否保存并回传名为 `hao_session` 的 Cookie。
- 会话是否超过固定 `auth.session_ttl_seconds`，或已经退出撤销。
- 用户 `status` 是否仍为 `active`。
- 使用 HTTPS 时 `auth.cookie_secure` 是否为 `true`；本地纯 HTTP 不应误设为 `true`。
- MySQL 模式是否连接到创建该会话的同一数据库；内存模式重启会丢失会话。
...
v2.0 的完整构建、干净目录和集成命令见 `docs/V2_ACCEPTANCE.md`，不能用历史 v1.1 构建结果替代当前证据。
...
从 Compose 依赖运行 MySQL / Redis 与 Kafka 完整回归：
...
回滚 005 会删除用户和会话，只能在备份和停写后执行：

```bash
bash tests/scripts/migrate_shortlink_schema.sh down --allow-data-loss
```

当前会话采用读取时判定过期，没有自动清理过期 / 撤销行。它不影响认证正确性，但长期运行应根据会话量
增加与最大 TTL 对齐的受控清理策略。
...
容器配置使用 Compose 内部地址 `kafka:29092`。v1.8 首版消费者只校验和记录事件；当前 v1.9 消费者关闭
自动 offset 提交，在统计事务或 DLQ delivery 成功后提交，并在提交持续失败时退出，避免继续消费后跨过失败 offset。
...
v2.0 使用统一迁移入口应用 001-005；不要再单独把 004 作为当前初始化方式。服务端仅在以下组合下注册统计 API：
...
统计 consumer 默认 group 为 `hao-shortlink-access-statistics-v1`，从 earliest 读取当前 topic 仍保留的事件。
健康和指标入口位于 consumer 的 `9091`；Compose 内可检查：

```bash
docker exec hao-shortlink-event-consumer curl -fsS http://127.0.0.1:9091/health
docker exec hao-shortlink-event-consumer curl -fsS http://127.0.0.1:9091/metrics
```

MySQL 处理成功或 DLQ delivery 确认后才提交源 offset。MySQL、DLQ 或 offset commit 达到有界重试上限时
进程非零退出，由 Compose 重启；HTTP redirect 和 server readiness 不依赖该 consumer。

受控重放必须先停止目标 group，再预览和执行 offset reset。不能对在线默认 group 直接试验：

```bash
docker exec hao-shortlink-kafka /opt/kafka/bin/kafka-consumer-groups.sh \
  --bootstrap-server kafka:29092 \
...
完整自动化验证使用独立 topic、group 和临时数据库，不清空当前投影：

```bash
bash tests/scripts/access_statistics_replay_test.sh
```

隔离重建只能覆盖 Kafka 当前 7 天 retention 范围。删除在线统计表或收据不属于自动回滚流程；应用回滚时
旧版本会忽略新增表，应先停止 v1.9 consumer，并在备份和重放水位明确后另行处理数据清理。

v2.0 的统计查询路径是需要登录和 owner 授权的 `/api/short-links/{code}/statistics`；旧 `/internal/` 路径
只作为 v1.9 历史设计记录，不再注册。

## v2.0 验收记录

当前功能、安全、迁移、浏览器、依赖故障和干净目录证据统一记录在 `docs/V2_ACCEPTANCE.md`。文档更新不能
替代命令输出；版本收口必须重新执行 Linux 构建、CTest、API、依赖集成、Compose 和契约检查。
