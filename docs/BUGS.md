...
补充保留日志和状态码分布的并发创建复现脚本，确认失败响应类型；按并发 1、2、4、8、16
逐步验证失败边界；排查 MySQL repository 事务逻辑、数据库连接池等待/复用行为和 HTTP 请求体解析在
并发 POST 下的稳定性。修复后需复跑 `mysql create` 和 `mysql-redis create` 基线，目标是错误率回到 0。
