...
## ENV-001：OrbStack VM 访问 Docker Redis 发布端口存在 IPv4 慢路径

状态：已定位，待验证配置规避
范围：本地 OrbStack VM 手工运行 `shortlink_server` 并访问 Docker Redis 发布端口的验证场景
...
v1.4.5 应复跑 Redis hit、Redis miss 回源、missing-code、Redis 不可用 fallback 和异常场景脚本。
在该环境问题完成配置验证和复测前，不直接把当前 0.2s 固定延迟归因为缺少 Redis 连接池。
