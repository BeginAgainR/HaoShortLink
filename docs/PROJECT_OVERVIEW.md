...
状态：已建立基础版，持续维护
当前实现：已实现健康检查、创建短链接、短码跳转、MySQL 持久化、可选 Redis 查询缓存和本地 Docker Compose 运行方式；v1.3 已完成第一版自动化测试和 CI，v1.4 已完成性能与稳定性基线，v1.5 已完成可观测性，v1.6 限流与健康语义已完成本地验证
...
- v1.6 已实现可配置的 Redis Lua 全局创建限流、fail-open、liveness / readiness、保护性测试和 CI 入口；云端 CI 待分支推送后确认。
- 完整部署方案尚未实现。
