...
3. 分段定位 `RedisShortLinkCache` 内部耗时，确认慢点来自连接、命令、释放、地址解析还是其他实现细节。
4. 根据分段结果决定是否实现 Redis 连接复用、连接池或调整 Redis 连接配置。
...
- 下一步需要分段定位 `RedisShortLinkCache` 内部耗时，再决定连接复用、连接池或连接配置调整方案。
