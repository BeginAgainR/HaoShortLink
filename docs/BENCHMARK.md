...
v2.2.5 发布前需要在既有 v1.x 相对基线上完成一次新的全链路性能与稳定性验收。该测试不用于宣传
生产最大承载能力，而是确认身份、Kubernetes、Kafka consumer 和 Outbox 加入后没有不可解释的退化或失控积压。

核心场景：

- Redis 命中短码跳转、Redis miss 后 MySQL 回源、短码不存在。
- 已认证用户创建随机 / 自定义短码、管理列表 / 详情、访问统计和生命周期审计查询。
- 读多写少的混合负载，并对比 Kubernetes 单副本与双副本。
- 访问事件 consumer 的消费能力，以及生命周期 relay / consumer 的稳定吞吐和积压恢复速度。

执行层级：

1. 预热后执行三轮固定请求基线，记录 QPS、平均延迟、P50、P95、P99 和错误率。
2. 阶梯增加并发，观察 CPU、内存、连接池、Kafka lag、outbox backlog 和最老待发布年龄。
3. 执行 30-60 分钟混合负载，确认资源、错误率、延迟和积压没有持续恶化。
4. 在负载中中断并恢复 Redis 或 Kafka，记录降级结果、恢复耗时和是否出现重复统计 / 审计。
5. 将 commit、VM / 集群资源、镜像、配置、数据规模、工具版本和原始结果随发布记录保存。

若 `hey` 无法表达登录、动态 token 或多步骤混合流量，可在 v2.0 收口时选择支持场景脚本的压测工具；
工具选择必须服务于可复现测试，不作为新增技术栈目标。详细通过条件见 `docs/FINAL_ACCEPTANCE.md`。
